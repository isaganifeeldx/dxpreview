    ...
    <section className="px-4 py-6 sm:px-6 lg:px-10">
      <div className="mx-auto grid max-w-[1350px] gap-5 lg:grid-cols-2">
        {promos.map((promo) => (
          <article
            key={promo.id}
            className="overflow-hidden rounded-[20px] border border-[#E2E6EE] bg-white p-7 sm:p-8"
          >
            <div className="max-w-[420px]">
              <h3 className="text-[22px] leading-snug font-semibold text-[#2A3040] sm:text-[26px]">
                {promo.title}
              </h3>
              <p className="mt-3 text-[14px] leading-relaxed text-[#6A758C]">{promo.description}</p>
              <Link
                href={promo.cta.href}
