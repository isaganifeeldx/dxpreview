  ...
  return (
    <div className="min-h-screen">
          ...
          {content.planFootnotes.length > 0 ? (
            <div className="mx-auto mt-8 max-w-[920px] space-y-3 px-1 text-center sm:mt-10">
              {content.planFootnotes.map((note) => (
                <p key={note} className="text-[12px] leading-relaxed text-[#6A758C] sm:text-[13px]">
                  {note}
                </p>
              ))}
            </div>
      ...
      <section className="px-4 py-14 sm:px-6 lg:px-10 lg:py-16">
        <div className="glass-panel mx-auto max-w-[1100px] !rounded-[16px] !p-8 text-center sm:!p-10">
          <p className="text-[15px] text-[#6A758C] sm:text-[16px]">{content.socialProof.title}</p>
          <div className="mt-8 flex flex-wrap items-center justify-center gap-x-8 gap-y-5 opacity-70 grayscale sm:gap-x-12">
            {content.socialProof.logos.map((logo) => (
              <span
                key={logo.name}
                className="lao-mn text-[18px] tracking-[0.08em] text-[#6A758C] sm:text-[20px]"
              >
                {logo.mark}
              </span>
            ))}
